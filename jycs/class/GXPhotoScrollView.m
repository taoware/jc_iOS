...
NSString * const GXPhotoScrollViewTappedNotification = @"GXPhotoScrollViewTappedNotification";

@interface GXPhotoScrollView () <UIScrollViewDelegate>
...
@implementation GXPhotoScrollView
...
- (void)displayImageAtIndex:(NSUInteger)index
{
    if ([self.dataSource respondsToSelector:@selector(photoAtIndex:)])
    {
        ...
        Photo *photo = [self.dataSource photoAtIndex:index];
        
        UIImage *image = [GXPhotoEngine imageForlocalPhotoUrl:photo.imageURL];
...
{
    [[NSNotificationCenter defaultCenter] postNotificationName:GXPhotoScrollViewTappedNotification object:recognizer];
