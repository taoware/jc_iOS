...
@implementation GXCarouselItem

@end

@implementation GXCarouselURLItem

@end

@implementation GXCarouselViewItem
...
@interface GXCarouselView () <UIScrollViewDelegate>
...
@property (nonatomic, copy) GXCarouselViewDidSelectBlock didSelectBlock;
@property (nonatomic, copy) GXCarouselViewDidChangeBlock didChangeBlock;

@end

@implementation GXCarouselView
...
- (void)layoutSubviews {
    ...
    frame.origin = CGPointMake(CGRectGetWidth(self.bounds) - frame.size.width + self.indicatorOffset.x,
...
-(void)setItems:(NSArray *)items {
    ...
    NSInteger index = 0;
    for (GXCarouselItem *item in _items) {
        DKClickableImageView *itemView = [DKClickableImageView new];

        itemView.userInteractionEnabled = YES;
        if ([item isKindOfClass:[GXCarouselURLItem class]]) {
            NSString *imageUrl = [(GXCarouselURLItem *)item imageUrl];
            [itemView setImageWithURL:[NSURL URLWithString:imageUrl] placeholderImage:self.defaultImage];
        } else if ([item isKindOfClass:[GXCarouselViewItem class]]) {
            UIView *customView = [(GXCarouselViewItem *)item view];
